...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    $query_get_old_file = "SELECT img FROM tb_instruktur WHERE id_instruktur = ?";
    ...
    if (move_uploaded_file($file_temp, $file_path)) {
        ...
        if ($stmt->execute()) {
            $query_update_konsumen = "UPDATE tb_instruktur SET name_instruktur = ?, nohp = ?, img = ? WHERE id_instruktur = ?";
            $stmt = $conn->prepare($query_update_konsumen);
            $stmt->bind_param("sssi", $nama, $nohp, $file, $id_user);
            $stmt->execute();
        ...
        if ($stmt->execute()) {
            $query_update_konsumen = "UPDATE tb_instruktur SET name_instruktur = ?, nohp = ? WHERE id_instruktur = ?";
            $stmt = $conn->prepare($query_update_konsumen);
            $stmt->bind_param("ssi", $nama, $nohp, $id_user);
            $stmt->execute();
            setNotification("Berhasil Mengedit Profile", "success");
            header("Location: settings.php");
            exit();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['upload'])) {
    $id_user = $_POST['id_user'];
    $file = $_FILES['img_ttd']['name'];
    $file_temp = $_FILES['img_ttd']['tmp_name'];
    $file_path = "img/" . $file;

    $query_get_old_file = "SELECT img_ttd FROM tb_instruktur WHERE id_instruktur = ?";
    $stmt = $conn->prepare($query_get_old_file);
    $stmt->bind_param("i", $id_user);
    $stmt->execute();
    $result = $stmt->get_result();
    $row = $result->fetch_assoc();
    $old_file = $row['img_ttd'];
    ...
    if (move_uploaded_file($file_temp, $file_path)) {
        ...
        $query_update_konsumen = "UPDATE tb_instruktur SET img_ttd = ? WHERE id_instruktur = ?";
        $stmt = $conn->prepare($query_update_konsumen);
        $stmt->bind_param("si", $file, $id_user);
        $stmt->execute();
        setNotification("Berhasil Mengupdate Tanda Tangan", "success");
        header("Location: settings.php");
        exit();
    }
...
$id_user = $_SESSION['id_user'];
$query = "SELECT * FROM tb_user INNER JOIN tb_instruktur ON tb_user.id_user = tb_instruktur.id_instruktur WHERE tb_user.id_user = ?";
...
<head>
    ...
    <title>Instruktur Dashboard | CV. Ratu Mengemudi</title>
    <!-- ======= Styles ====== -->
            ...
            <div style="display: flex; justify-content: center; align-items: center;">
                <div class="card"
                    ...
                    <div class="cardHeader">
                        <center>
                            <?php if ($row['img'] == NULL) { ?>
                                <img src="../img/instruktur.png" alt="Foto Profil" width="60" height="60">
                            <?php } else { ?>
                        ...
                        <form action="settings.php" method="post" class="form-container" enctype="multipart/form-data">
                            ...
                            <input class="form-input" type="text" placeholder="Masukan Nama" id="nama" name="nama"
                                value="<?php echo $row['name_instruktur']; ?>" required>
                        ...
                        <br><br>
                        <center>
                            <?php if ($row['img_ttd'] == NULL) { ?>
                                <img src="../img/instruktur.png" alt="Foto Profil" width="60" height="60">
                            <?php } else { ?>
                                <img src="img/<?php echo $row['img_ttd'] ?>" alt="Foto Profil" width="60" height="60">
                            <?php } ?>
                        </center>
                        <form action="settings.php" method="post" class="form-container" enctype="multipart/form-data">
                            <label for="">Tanda Tangan: </label>
                            <input class="form-input" type="file" id="img_ttd" name="img_ttd">
                            <input type="hidden" name="id_user" value="<?php echo $row['id_user']; ?>">
                            <button class="btn-submit" type="submit" name="upload">
                                Upload
                            </button>
                        </form>
                    </div>
